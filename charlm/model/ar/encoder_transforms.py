...
class ConditionalSplineAutoregressive1d(ConditionalAutoregressiveBijection):
    def __init__(
        self, c, num_layers, hidden_size, dropout, num_bins, context_size, unconstrained
    ):
        self.unconstrained = unconstrained
        self.num_bins = num_bins
        scheme = InvertSequentialCL(order="cl")
        lstm = ConditionalAutoregressiveLSTM(
            C=c,
            P=self._num_params(),
        ...
        super(ConditionalSplineAutoregressive1d, self).__init__(
            ar_net=lstm, scheme=scheme
        )
    ...
    def _element_inverse(self, z, element_params):
        ...
        unnormalized_derivatives = (
            element_params[..., 2 * self.num_bins :] + self.constant
        )
        if self.unconstrained:
...
class ConditionalAutoregressiveLSTM(nn.Module):
    def __init__(self, C, P, num_layers, hidden_size, dropout, context_size):
        ...
        self.l_in = PermuteLayer((2, 0, 1))  # (B,C,L) -> (L,B,C)
        self.lstm = ConditionalLayerLSTM(
            C + context_size, hidden_size, num_layers=num_layers, dropout=dropout
        )  # (L,B,C) -> (L,B,H)
        self.l_out = nn.Sequential(
            nn.Linear(hidden_size, P * C),  # (L,B,H) -> (L,B,P*C)
            AutoregressiveShift(P * C),
            ReshapeLayer(C, P),  # (L,B,P*C) -> (L,B,C,P)
            PermuteLayer((1, 2, 0, 3)),  # (L,B,C,P) -> (B,C,L,P)
        )
...
class ConditionalLayerLSTM(nn.LSTM):
    def forward(self, x, context):
        output, _ = super(ConditionalLayerLSTM, self).forward(
...
class ReshapeLayer(nn.Module):
    ...
    def forward(self, x):
        return x.reshape(*x.shape[0:2], self.C, self.P)
