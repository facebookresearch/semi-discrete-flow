...
class MADE(nn.Module):
    ...
    def update_masks(self):
        ...
        self.m[-1] = np.concatenate([self.m["out"], torch.zeros(self.ncond)])
        for l in range(L):
...
if __name__ == "__main__":
    ...
    x = (rng.rand(1, D + 2) > 0.5).astype(np.float32)
    ...
    for nin, hiddens, nout, natural_ordering in configs:
        ...
        model = MADE(nin, hiddens, nout, ncond=2, natural_ordering=natural_ordering)
