...
__all__ = ["iResBlock", "residual_block_fn"]
...
class iResBlock(nn.Module):
    ...
    def forward(self, x, *, logp=None, cond=None, **kwargs):
        if logp is None:
            y = x + self.eval_nnet(x, cond)
            return y
        else:
            g, logdetgrad = self._logdetgrad(x, cond=cond)
            return x + g, logp - logdetgrad

    def inverse(self, y, *, logp=None, cond=None, **kwargs):
        x = self._inverse_fixed_point(y, cond)
        if logp is None:
            return x
        else:
            return x, logp + self._logdetgrad(x)[1]

    def _inverse_fixed_point(self, y, cond=None, atol=1e-5, rtol=1e-5):
        x, x_prev = y - self.eval_nnet(y, cond), y
        i = 0
        tol = atol + y.abs() * rtol
        while not torch.all((x - x_prev) ** 2 / tol < 1):
            x, x_prev = y - self.eval_nnet(x, cond), x
            i += 1
            if i > 1000:
    ...
    def eval_nnet(self, x, cond):
        if cond is None:
            return self.nnet(x)
        else:
            return self.nnet(x, cond)

    def _logdetgrad(self, x, cond=None):
            ...
            if not self.exact_trace:
                ...
                if self.training and self.grad_in_forward:
                    g, logdetgrad = mem_eff_wrapper(
                        estimator_fn,
                        partial(self.eval_nnet, cond=cond),
                        x,
                        n_power_series,
                        vareps,
                        coeff_fn,
                        self.training,
                        list(self.nnet.parameters()),
                    )
                else:
                    x = x.requires_grad_(True)
                    g = self.eval_nnet(x, cond)
...
def mem_eff_wrapper(
    estimator_fn, gnet, x, n_power_series, vareps, coeff_fn, training, parameters
):
...
from .base import get_linear, get_conv2d
from .base.activations import LipSwish


class ConditionalLayer(nn.Module):
    def __init__(self, lipschitz_layer, cond_layer):
        super().__init__()
        self.lipschitz_layer = lipschitz_layer
        self.cond_layer = cond_layer

    def forward(self, x, cond):
        return self.lipschitz_layer(x) + self.cond_layer(cond)


class ConditionalSequential(nn.Module):
    def __init__(self, *mods):
        super().__init__()
        self.modulelist = nn.ModuleList(mods)

    def forward(self, x, cond=None):
        for mod in self.modulelist:
            if isinstance(mod, ConditionalLayer):
                x = mod(x, cond)
            else:
                x = mod(x)
        return x
...
def residual_block_fn(
    ...
    **kwargs,
    ...
    # not tested for fc=True
    assert not fc

    layer_fn = get_linear if fc else get_conv2d

    first_layer = layer_fn(
        input_size[0],
        idim,
        3,
        1,
    ...
    if cond_embed_dim > 0:
        first_layer = ConditionalLayer(
            first_layer, nn.Conv2d(cond_embed_dim, idim, 1, 1, 0),
        )

    nnet = ConditionalSequential(
        LipSwish(),
        first_layer,
        LipSwish(),
        layer_fn(
