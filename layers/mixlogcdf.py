...
"""
# Modifed from: https://github.com/visinf/mar-scf.
...
def mixture_log_pdf(x, prior_logits, means, log_scales, dim):
    """Log PDF of a mixture of logistic distributions."""
    log_ps = F.log_softmax(prior_logits, dim=dim) + _log_pdf(
        x.unsqueeze(dim), means, log_scales
    )
    log_p = torch.logsumexp(log_ps, dim=dim)

    return log_p


def mixture_log_cdf(x, prior_logits, means, log_scales, dim):
    """Log CDF of a mixture of logistic distributions."""
    log_ps = F.log_softmax(prior_logits, dim=dim) + _log_cdf(
        x.unsqueeze(dim), means, log_scales
    )
    log_p = torch.logsumexp(log_ps, dim=dim)
...
def mixture_inv_cdf(y, prior_logits, means, log_scales, dim, eps=1e-10, max_iters=100):
    """Inverse CDF of a mixture of logisitics. Iterative algorithm."""
    if y.min() <= 0 or y.max() >= 1:
        raise RuntimeError("Inverse logisitic CDF got y outside (0, 1)")

    def body(x_, lb_, ub_):
        cur_y = torch.exp(mixture_log_cdf(x_, prior_logits, means, log_scales, dim=dim))
    ...
    max_scales = torch.sum(torch.exp(log_scales), dim=dim, keepdim=True)
    lb, _ = (means - 20 * max_scales).min(dim=dim)
    ub, _ = (means + 20 * max_scales).max(dim=dim)
    diff = float("inf")
...
def sigmoid_inverse(x):
    ...
    z = -safe_log(x.reciprocal() - 1.0)
    ldj = -safe_log(x) - safe_log(1.0 - x)
    return z, ldj
