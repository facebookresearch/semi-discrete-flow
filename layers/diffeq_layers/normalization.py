...
class BatchNorm(nn.Module):
    ...
    def __init__(self, num_features):
        ...
        self.affine = ContinuousAffine(basis=basis, shape=(None, num_features, None, None), dims=(1,))
    ...
    def __init__(self, shape, dims, extra_linear_shape):
        ...
        for i, s in enumerate(shape):
            if i in dims and not isinstance(s, int):
                raise ValueError("The {}-th dimension is a dimension to compute an affine transformation over, so an "
                                 "integer value for its size must be provided.".format(i))

        self.shape = shape
        self.dims = dims
        linear_shape = [s if i in dims else 1 for i, s in enumerate(shape)] + extra_linear_shape
...
class ContinuousAffine(AffineBase):
    def __init__(self, basis, **kwargs):
        super(ContinuousAffine, self).__init__(**kwargs, extra_linear_shape=[basis.basis_size])
        self.basis = basis
...
class Legendre(Basis):
    def __init__(self, basis_size, start, end, **kwargs):
        ...
        self.register_buffer('arange', torch.arange(1, 2 * basis_size, 2, dtype=torch.float32).sqrt())
    ...
    def _forward(self, t):
        ...
        if self.basis_size > 1:
            ...
            for i in range(1, self.basis_size - 1):
                value = ((2 * i + 1) * scaled_t * retvar[..., i] - i * retvar[..., i - 1]) / (i + 1)
...
class Fourier(Basis):
    def __init__(self, basis_size, start, end, **kwargs):
        ...
        self.register_buffer('arange', torch.arange(1, (basis_size + 1) // 2) * 2 * math.pi / (end - start))

    def _forward(self, t):
        # t can have arbitrary shape
        scaled_t = (t - self.start).unsqueeze(-1) * self.arange
        return torch.cat([torch.ones(*t.shape, 1, dtype=t.dtype, device=t.device), scaled_t.cos(), scaled_t.sin()], dim=-1)
